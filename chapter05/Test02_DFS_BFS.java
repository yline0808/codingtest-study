...
public class Test02_DFS_BFS {
    ...
    public static void adjacencyList() {
        ArrayList<ArrayList<Node02>> graph = new ArrayList<ArrayList<Node02>>();

        for (int i = 0; i < 3; i++) {
            graph.add(new ArrayList<Node02>());
        }

        graph.get(0).add(new Node02(1, 7));
        graph.get(0).add(new Node02(2, 5));
        graph.get(1).add(new Node02(0, 7));
        graph.get(2).add(new Node02(0, 5));
